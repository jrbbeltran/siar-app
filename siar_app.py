...
# ============================================================
CSII_SCORES_URL  = "https://drive.google.com/uc?export=download&id=1Ul593NIQhJ3yjUqs6GNEtXDOv1IRtSX3"
CSII_DATA_URL    = "https://drive.google.com/uc?export=download&id=1z5pswy038NH_0DG2Kt7iMT0DAd7pb4RY"
INSTITUTIONS_URL = "https://drive.google.com/uc?export=download&id=1KrKfJTqSRjiOEFLZHrJWiGDI6s8Xx8F0"

# ============================================================
# GOOGLE DRIVE LOADER (handles large files)
# ============================================================
...
@st.cache_data(show_spinner=False)
def load_core_data():
    scores = load_csv_from_gdrive(CSII_SCORES_URL)
...
def get_interpretation(siar_value):
    if siar_value >= 6: return "MVP-caliber impact"
    elif siar_value >= 4: return "All-Star level"
    elif siar_value >= 2: return "Solid starter"
    elif siar_value > 0: return "Above replacement"
    else: return "At or below replacement level"
...
try:
    ...
    st.error("Failed to load data from Google Drive. Please check the links.")
    st.stop()
...
with st.sidebar:
    ...
    window_years = st.slider("Years window", 3, 10, 5)
    end_date = st.date_input("End date", value=datetime(2026, 7, 7))

tab1, tab2, tab3 = st.tabs(["Author SIAR", "Institution", "Methodology"])

with tab1:
    st.subheader("Author Lookup")
    search_id = st.text_input("author_id (best)", placeholder="e.g. 6506343587")
...
with tab2:
    st.info("Institution dashboard coming soon.")

with tab3:
    st.markdown("**SIAR** = max(0, (h₅ − RL) / s)")

st.caption("siar.app • CSII 2.0 • Academic use only")